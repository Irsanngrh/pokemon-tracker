...
const saveCard = (card) => {
    ...
    router.post(`/admin/cards/${card.id}?key=${props.adminKey}`, {
        price: currentPrice,
    ...
    <AppLayout>
        <div class="space-y-6">
            <div>
                ...
                <p class="text-gray-500 dark:text-gray-400">Update card prices and image URLs manually.</p>
            </div>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                            <tr v-for="card in cards" :key="card.id">
                                ...
                                <td class="px-6 py-4">
                                    <input 
                                        type="number" 
                                        v-model="card.price.price" 
                                        v-if="card.price"
                                        class="w-32 px-3 py-1.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md"
                                    />
                                    <input 
                                        type="number" 
                                        v-else
                                        @input="(e) => card.price = { price: e.target.value }"
                                        class="w-32 px-3 py-1.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md"
                                        placeholder="0"
                                    />
                                </td>
                                <td class="px-6 py-4">
                                    <input 
                                        ...
                                        class="w-64 px-3 py-1.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md"
                                    />
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <button 
                                        ...
                                        class="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md disabled:opacity-50 transition"
                                    >
                                        {{ processingId === card.id ? 'Saving...' : 'Save' }}
                                    </button>
                                </td>
                            </tr>
                        </tbody>
